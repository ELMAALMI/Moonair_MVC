...
function goingNext(method)
{
    ...
    if(method == "")
        xhttp.open("GET","AjaxSearchController?key="+last.innerHTML+"&way=next&line_num="+line_infos.dataset.line+"&line_way="+line_infos.dataset.way+"&method="+method+"&tr=true",true);
    else
        xhttp.open("GET","AjaxSearchController?key="+last.innerHTML+"&way=next&line_num="+line_infos_return.dataset.line+"&line_way="+line_infos_return.dataset.wayreturn+"&method="+method+"&tr=true",true);
...
function goingPrevious(method)
{
    ...
    if(method == "")
            xhttp.open("GET","AjaxSearchController?key="+first.innerHTML+"&way=prev&line_num="+line_infos.dataset.line+"&line_way="+line_infos.dataset.way+"&method="+method+"&tr=true",true);
    else
            xhttp.open("GET","AjaxSearchController?key="+first.innerHTML+"&way=prev&line_num="+line_infos_return.dataset.line+"&line_way="+line_infos_return.dataset.wayreturn+"&method="+method+"&tr=true",true);
...
function flightDetails(flight_num, method)
{
    ...
    if(method != "_return")
        xhttp.open("GET","AjaxSearchController?flight="+flight_num+"&line_num="+line_infos.dataset.line+"&line_way="+line_infos.dataset.way+"&method="+method+"&tr=true",true);
    else
        xhttp.open("GET","AjaxSearchController?flight="+flight_num+"&line_num="+line_infos_return.dataset.line+"&line_way="+line_infos_return.dataset.wayreturn+"&method="+method+"&tr=true",true);
    xhttp.send();
    if(method != "_return")
...
function getNationality()
{
    ...
    xhttp.onreadystatechange=function()
    {
        if (this.readyState==4 && this.status==200)
        {
            ...
            {
               tab.push(JSON.parse(this.responseText)[i].country);
            }
        }
    }
    xhttp.open("GET","AjaxSearchController?nationality=true",true);
    xhttp.send();
    return tab;
}
/* ################################################## 
   ...
   ################################################## */
   console.log(Math.ceil(43/12))
var selectcontry = getNationality();
selectcontry.forEach(element => {
    console.log(element)
});
for(var i=0; i< adults; i++)
{
    ...
    $(".passangers-details").append("<div class='passangers-infos' id='adult-info-"+(i+1)+"' "+((i!=0) ? "style='display: none'" : "")+" '>\
                                        ...
                                        <div class='passenger-form'>\
                                            ...
                                            <p>Birthday <span style='color: red'>*</span></p>\
